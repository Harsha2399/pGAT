...
class Corpus:
    ...
    def get_validation_pred(self, args, model, unique_entities):
        ...
        for iters in range(1):
            ...
            preds=[]
            pred = batch_indices.tolist()
            pred1 = batch_indices.tolist()
            ...
            for i in range(batch_indices.shape[0]):
                ...
                # pred = new_x_batch_head[:,:][0].tolist()
                # pred1 = new_x_batch_tail[:,:][0].tolist()
                ...
                if 'wn' in args.data:
                    ...
                    scores_head = torch.cat(
                        [scores1_head, scores2_head, scores3_head, scores4_head,scores5_head, scores6_head, scores7_head, scores8_head,scores9_head, scores10_head], dim=0)
                    scores_head = torch.sigmoid(scores_head)
                else:
                    #scores_head = torch.sigmoid(model.batch_test(new_x_batch_head))
                    scores_head = (model.batch_test(new_x_batch_head))
                print(scores_head.shape)
                sorted_scores_head, sorted_indices_head = torch.sort(
                    scores_head.view(-1), dim=-1, descending=True)
                sorted_scores_head1, sorted_indices_head1 = torch.sort(
                    scores_head, dim=1, descending=True)
                # Just search for zeroth index in the sorted scores, we appended valid triple at top
                #ranking = (sorted_indices_head [i, :] == entity_list[i]).nonzero()
                #print(sorted_scores_head1.shape)
                #print(sorted_scores_head.shape)
                #for j in range(len(sorted_scores_head1)):

                ranking = np.where(sorted_indices_head.cpu().numpy() == 0)[0][0]
                #ranking = sorted_indices_head1[j:]==positivearg[j].nonzero()
                pred[i].append('h')
                pred[i].append(ranking + 1)
                ls = zip(sorted_indices_head[0:100].data.cpu().numpy().tolist(),sorted_scores_head[0:100].data.cpu().numpy().tolist())
                pred[i].append(ls)
                ranks_head.append(
                    np.where(sorted_indices_head.cpu().numpy() == 0)[0][0] + 1)
                reciprocal_ranks_head.append(1.0 / ranks_head[-1])
                #print("Head-ranking",ranking+1)
                #preds+=pred[i]
                # Tail part here
                ...
                if 'wn' in args.data:
                    ...
                    scores_tail = torch.cat(
                        [scores1_tail, scores2_tail, scores3_tail, scores4_tail,scores5_tail, scores6_tail, scores7_tail, scores8_tail,scores9_tail, scores10_tail], dim=0)
                    scores_tail = torch.sigmoid(scores_tail)

                else:
                    #scores_tail = torch.sigmoid(model.batch_test(new_x_batch_tail))
                ...
                sorted_scores_tail1, sorted_indices_tail1 = torch.sort(
                    scores_tail, dim=1, descending=True)

                # Just search for zeroth index in the sorted scores, we appended valid triple at top
                #ranking = (sorted_indices_tail [i, :] == entity_list[i]).nonzero()
                #for j in range(len(sorted_scores_tail1)):
                ranking = np.where(sorted_indices_tail.cpu().numpy() == 0)[0][0]
                pred1[i].append('t')
                pred1[i].append(ranking+1)
                ls = zip(sorted_indices_tail[0:100].data.cpu().numpy().tolist(), sorted_scores_tail[0:100].data.cpu().numpy().tolist())
                #f=sorted_indices_tail[0].data.cpu().numpy().tolist()
                #print()
                pred1[i].append(ls)
                ranks_tail.append(
