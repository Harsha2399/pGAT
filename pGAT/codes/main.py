...
from preprocess import read_entity_from_id, read_relation_from_id, init_embeddings, build_data,load_data1
...
def parse_args():
    ...
    args.add_argument("-data", "--data",
                      default="/home/harsha1/Desktop/pLogicNet/data/wn18rr/", help="data directory")
    ...
    args.add_argument("-work_path","--work_path",help="work directory")
...
def load_data(args):
    train_data, validation_data, test_data, entity2id, relation2id, headTailSelector, unique_entities_train = build_data(args.work_path,
...
def evaluate_conv(args, unique_entities):
    ...
    cand_triples, train_adjacency_mat, unique_entities_test = load_data1(os.path.join(
        work_path, 'hidden.txt'), entity2id, relation2id, is_unweigted, directed)
...
def infer_step(model, infer_triples, args):
    ...
    for k in range(0, len(infer_triples), batch_size):
        ...
        batch = batch.cuda()
        score = torch.sigmoid((model.batch_test(batch))).view(-1)
