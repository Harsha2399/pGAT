...
def load_data1(filename, entity2id, relation2id, is_unweigted=False, directed=True):
...
def build_data(work_path,path, is_unweigted=False, directed=True):
    ...
    train_triples, train_adjacency_mat, unique_entities_train = load_data1(os.path.join(
        work_path, 'train_kge.txt'), entity2id, relation2id, is_unweigted, directed)
    validation_triples, valid_adjacency_mat, unique_entities_validation = load_data1(
        os.path.join(path, 'valid.txt'), entity2id, relation2id, is_unweigted, directed)
    test_triples, test_adjacency_mat, unique_entities_test = load_data1(os.path.join(
    ...
    with open(os.path.join(work_path, 'train_kge.txt')) as f:
        lines = f.readlines()
